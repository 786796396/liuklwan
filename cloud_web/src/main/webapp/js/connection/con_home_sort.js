...
function TableSorter(table) {
	...
	this.tabSort=document.getElementById(this.Table.id+"Sort");
    ...
    if (args.length > 1) {
        for (var x = 1; x < args.length; x++) {
            ...
            if (args[x] > this.Table.rows[0].cells.length) {
                ...
                this.tabSort.rows[0].cells[args[x]].onclick = this.GetFunction(this, "Sort", args[x]);
                this.tabSort.rows[0].cells[args[x]].style.cursor = "pointer";
            }
        }
    }
    // 参数不大于1，说明所有的字段都需要排序 
        ...
        for (var x = 0; x < this.Table.rows[0].cells.length; x++) {
            this.tabSort.rows[0].cells[x].onclick = this.GetFunction(this, "Sort", x);
            this.tabSort.rows[0].cells[x].style.cursor = "pointer";
        }
    }
}
...
TableSorter.prototype.Sort = function(col) {
    ...
    // 为表头设置样式 
    for (var x = 0; x < this.Table.rows[0].cells.length; x++) {
        this.tabSort.rows[0].cells[x].className = this.NormalCss;
    }
    // 定义放置需要排序的行数组 
    var Sorter = [];
    for (var x = 0; x < this.Table.rows.length; x++) {
		 if ($(this.Table.rows[x].cells[col]).find(".sort-num").length > 0) {
            Sorter[x] = [$(this.Table.rows[x].cells[col]).find(".sort-num").html(), x];
        } else {
            Sorter[x] = [this.Table.rows[x].cells[col].innerHTML, x];
        }
    ...
    if (this.ViewState[col]) {
        ...
        this.tabSort.rows[0].cells[col].className = this.SortDescCss;
    } else {
        this.ViewState[col] = true;
        this.tabSort.rows[0].cells[col].className = this.SortAscCss;
    }
    var Rank = [];
    for (var x = 0; x < Sorter.length; x++) {
        Rank[x] = this.GetRowHtml(this.Table.rows[Sorter[x][1]]);
    }
    if (this.Table.getAttribute("_hasNum") == "true") {
        for (var x = 0; x < Sorter.length; x++) {
            Rank[x][0] = x + 1;
        }
    }
    ...
    keySearchCommonSort('searchInfo');
    keySearchCommonSpotSort($(this.Table).attr("value"));//大数据分析
}
...
function keySearchCommonSort(id) {
    var searchInfo = $("#" + id).val();
    if (searchInfo == "") {
        $("#listOrg tr").show();
    } else {
        $("#listOrg").find(".wz-name").each(function(index, element) {
...
//大数据分析
function keySearchCommonSpotSort(id) {
	var searchInfo = $("#" + id).val();
	if (searchInfo == "") {
		$("[name='tableName'] tr").show();
	} else {
		$("[name='tableName']").find(".wz-name").each(function(index, element) {
			if ($(this).html().indexOf(searchInfo) < 0) {
				$(this).parents("tr").hide();
			} else {
				$(this).parents("tr").show();
