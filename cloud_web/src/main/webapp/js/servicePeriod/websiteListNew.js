...
function getSpotResult(){
	...
	//抽查
	if(toType == 0){
		urlStr="/spotCheckResult_getSpotResult.action"+"?scheduleId="+scheduleId+"&servicePeriodId="+servicePeriodId;
	}else{
		...
		success:function(data){
			...
			if(data.listSize==0){
				...
				listSumStr+="<tr class='summary_tr' >" +
					   "<td colspan='2' class='summary_td' style='color: #4a5b6c;width:137px'>汇总 <i></i></td>" +
...
//					"<td><input type='checkbox' /></td>" ;
					//报告 已完成   	整改 未通知
					 if(obj.isDown==1 && servicePeriodStatus!=0 && obj.isDown==1 && obj.checkReportResult==0){
						 //可以选择
						 listStr+="<td style='width:20px;'><input type='checkbox' name='chk_list' value='"+siteCode+"'/></td>";
					 }else{
						 //不可选择 disabled
						 listStr+="<td style='width:20px;'><input type='checkbox' value='"+siteCode+"' disabled/></td>";
					 }
//					wz-name td_left font_701999
					 listStr+="<td class='wz-name' style='width:90px;'><a onclick='fillJumpTb()' target='_blank' class='keykey colo_1ca8dd_tn' href='"+webPath+"/fillUnit_gailan.action?siteCode="+obj.siteCode+"'>"+obj.siteCode+"</a>" +
					...
					if(toType != 0){
						...
						if(endDate > quDate){
							listStr+="<td style='width:36px;'><span>"+obj.siteCheckResultName+"</span></td>" ;
						}
					}
...
//						class='colo_1ca8dd_tn'
				...
				if(loginSiteCode.length==6){
					...
					if(servicePeriodStatus!=0 && obj.isDown==1 && obj.checkReportResult==0){
					    ...
					    "<span class='span_t'>未通知</span><span class='gzzg-blue span_t' onclick=\"spotPass('"+siteCode+"','"+isDown+"')\" data-toggle='modal' data-target='#myModalNotice'>通知整改<input type='hidden' value='"+obj.isDown+"'></span></td></tr>";
						...
						"<span class='span_t'>通知整改</span></td></tr>";
					}
                    }
...
function batchReport(){
	...
	if(reportIsDown == 1){
		$.ajax({
	        ...
	        type: 'POST',
	        dataType: "json",
	        url: webPath + "/spotCheckResult_batchReport.action",
	        success: function (data) {
	        	if(data.isDown == 0){
	        		alert(data.errorMsg);
	        	}else{
	        		$("#filePathReport").val(data["filePath"]);
					$("#fileNameReport").val(data["fileName"]);
					if($("#filePathReport").val() != ""){
						$("#dowFormReport").submit();
					}
//	        		window.location.href = webPath + "/spotCheckResult_batchReport.action?servicePeriodId="+servicePeriodId+"&siteCodes="+siteCodes;
	        	}
	...
	$("#moreOperation").css('display','none');
}
