...
$(function () {
   ...
   // dailyStatistics();
    ...
    var timer=null;
    $('.third_part .content h4').mouseover(function(){
        clearTimeout(timer);
        var obj = $(this);
    ...
    $('.third_part .content h4').mouseout(function(){
        clearTimeout(timer);
        var obj = $(this);
        obj.popover('hide');
    });
...
//跳转到全屏
function fullScreen(){
	//window.open(webPath+"/siteDataOverview_fullScreen.action", "_blank","fullscreen");
		window.location.href=webPath+"/siteDataOverview_fullScreen.action";
}
//获取问题播报数据
...
function getProblems(){
	$.ajax({  
		type : "POST",    
		url : webPath+"/siteDataOverviewChart_getBroadcastDatas.action",
		dataType:"json",
		async : false,  
...
function getVisitData(){
	...
	$.ajax({  
		...
		url : webPath+"/siteDataOverviewChart_getJcVisitOrg.action", 
		dataType:"json",
		async : false,  
		success : function(data) {
			if(data.success == 'true'){
...
function healthIndex(menuType){
	...
	$.ajax({  
		type : "POST",    
//		url : webPath+"/indexCount_getIndexSumMTask.action", 
		url : webPath+"/siteDataOverviewChart_indexCountOrg.action",
		dataType:"json",
		data:"menuType="+menu,
		async : false,  
		success : function(data) {
			...
			//健康指数
			var totalSumNumber=data.healthIndex;
...
//		url : webPath+"/fillUnit_indexCountLineBM.action", 
		url : webPath+"/siteDataOverviewChart_indexCountLineBM.action",
		dataType:"json",
		async : false,  
		success : function(data) {
...
function checkDatas(){
	$.ajax({  
        ...
        url : webPath+"/siteDataOverviewChart_getSiteStateInfo.action",  
        dataType:"json",
        async : false,  
        success : function(data) {
        	if(data.success == 'true'){
        		//总数，因为DicConfig中统计了全部的监测站点数，且在多次有调用，所以bm0100监测站点数为DicConfig的的数据，其他省的都是通过tree表与Info表进行查询所得
        		var count = 0;
        		...
        		var sumAll = data.info.sumAll;
        		var sumClose = data.info.sumClose;
        		var sumNormal = data.info.sumNormal;
        		var liWai = sumAll - sumClose - sumNormal;
        		var zhengChangPer=((sumNormal/sumAll)*100).toFixed(2);
        		var guanTingPer = ((sumClose/sumAll)*100).toFixed(2);
...
function healthMap(){
	...
	$.ajax({  
       ...
       // url :  webPath+"/siteDataOverview_dailyMap.action", 
        url :  webPath+"/siteDataOverviewChart_getDailyMap.action", 
        dataType:"json",
        async : false,  
        success : function(dat) {
        	if(dat){
	        	...
	        	$("#linkErrSiteProp_span").width(maxAndMin(linksuccsiteprop)+"%");
	     		...
	     		var prophtml=maxAndMin(updatesiteprop)+"<span>%</span>";
	     		$("#noUpdateSiteProp_h").html(prophtml);
	        	$("#noUpdateSiteProp_span").width(maxAndMin(updatesiteprop)+"%");
        		...
        		$.each(dat.body, function(index, obj) {
        			...
        			console.info(name+"==="+linkerrsiteprop+'%');
 ...
 $.get(webPath +"/js/map/provinceJson/"+ loginSiteCode +".json",function(obj){
	 echarts.registerMap('henan', obj);
	  ...
	  chart.setOption({
	      ...
	      series : [
	          ...
	          
	          {
	              ...
	              type: 'effectScatter',
	              coordinateSystem: 'geo',
	              data: convertData95(data.sort(function (a, b) {return parseFloat(b.value) - parseFloat(a.value);}).slice(0, 5)),
	              symbolSize: function (val) {
...
function toBigDateJsp(){
	changeCookie(1,webPath + "/siteDataOverview_siteDataOverviewChart.action",null,
					webPath + "/dailyMonitoringStatistics_dailyMonitoringStatistics.action");
...
function toHomeJsp(){
	changeCookie(1,webPath + "/siteDataOverview_siteDataOverviewChart.action",null,
			webPath + "/siteDataOverview_siteDataOverview.action");
