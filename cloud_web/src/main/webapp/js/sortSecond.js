...
TableSorter.prototype.Sort = function(col) {
    ...
    for (var x = 1; x < this.Table.rows.length; x++) {
        ...
        if ($(this.Table.rows[x].cells[col]).find(".sort-num").length > 0) {
            Sorter[x - 1] = [$(this.Table.rows[x].cells[col]).find(".sort-num").html(), x];
        } else {
            Sorter[x - 1] = [this.Table.rows[x].cells[col].innerHTML, x];
        }
        // alert(Sorter[x-1]); 
    ...
    keySearchCommonSort($(this.Table).find("tbody").attr("id"));
}
...
function keySearchCommonSort(id) {
    var searchInfo = $("#" + id + "_term").val();
    if (searchInfo == "") {
        $("#" + id + " tr").show();
    } else {
        $("#" + id).find(".key").each(function(index, element) {
