...
<script type="text/javascript" src="<%=path%>/js/securityGuarantee/securityGuaranteeMain.js"></script>
...
<div class="main-container container">
	...
	<div class="row-fluid">
        <c:set var="ba_index" value="305" scope="request" />
                    ...
                    <div class="fl select-box">
			           ...
			           <div class="fl period-box">
                        ...
                        <div class="fl select-box select-box-long serviceOne">
                        	<span id="aaaTypeText">${nearServicePeriod}</span>
                        	<input type="hidden" id="nearServicePeriodId" value="${nearServicePeriodId }">
                        	<input type="hidden" id="servicePeriodIdVal">
                        	<input type="hidden" id="servicePeriodIdText">
                            <i></i>
                            <!--移入显示块开始-->
                            <ul id="servicePeriodIdUl" style="display: none;">
                                <c:forEach items="${servicePeriodListResult}" var="sp">
									 <li name="servicePeriodId" id="servicePeriodId" value="${sp.id}" >${sp.startDate}至${sp.endDate}</li>
								</c:forEach>
                            </ul>
                            <!--移入显示块结束-->
                        </div>
                        <div class="fl select-box select-box-long serviceTwo">
                        	 <span id="aaaTypeTextBasic">${nearServicePeriodBasic}</span>
                        	 <input type="hidden" id="nearServicePeriodBasicId" value="${nearServicePeriodBasicId }">
                        	<input type="hidden" id="servicePeriodIdValBasic">
                        	<input type="hidden" id="servicePeriodIdTextBasic">
                            <i></i>
                            <!--移入显示块开始-->
                            <ul id="servicePeriodIdUlBasic" style="display: none;">
                                <c:forEach items="${servicePeriodRequestBasic}" var="spBasic">
									 <li name="servicePeriodIdBasic" id="servicePeriodIdBasic" value="${spBasic.id}" >${spBasic.startDate}至${spBasic.endDate}</li>
								</c:forEach>
                            </ul>
                            <!--移入显示块结束-->
                        </div>
    ...
    $(function(){
        ...
        selectShow("aaaType");
		 ...
		 $("#servicePeriodIdUlBasic li").click(function(){
			...
			$("#aaaTypeTextBasic").html($(this).html());
			changeFunction();
		});
		
		$("#siteTypeIdUl li").click(function(){     //============点击逾期或者正常 显示不同的红字
