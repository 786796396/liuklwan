    ...
    $(function(){
        ...
        formYJ=$("#formYJ_id").val();
        //页面数据初始化
        initSecurityServiceTbData();
    });
    
    function initSecurityServiceTbData(){
        //清空列表数据
        $("#security_blank_tb_id").html("");
        // dropload
        $("#security_blank_tb_list_id").dropload({
            scrollArea : window,
            loadDownFn : function(me){
				$.ajax({
			   	 	type:"post",
			     	async:false,
			        url: webPath+"/advanceToken_getSecurityBlankTbData.action?siteCode="+siteCode
			        			+"&servicePeroidId="+servicePeroidId
			        			+"&pageNum="+pageNum+"&pageSize="+pageSize+"&formYJ="+formYJ,
			        dataType:"json",
			        	   ...
			        	   if(returnList && returnList.length>0){
			        		   for ( var i = 0; i < returnList.length; i++) {
			        			   dataStr+="<li class='clearfix'><i class='fl'></i><div class='fl right-part'><div class='clearfix'>"
			        				   ...
			        				   +"<p class='clearfix'><span class='title-txt-f fl'>描述\：</span><span class='fl except-tit'>"+returnList[i]["questionDescribe"]+"</span></p>"
			        				   +"<p class='clearfix'><span class='title-txt-f fl'>路径\：</span><a href='"+returnList[i]["url"]+"'><span class='fl except-tit'>"+returnList[i]["url"]+"</span></a></p></div></li>";
	                           ...
	                           // 为了测试，延迟1秒加载
	                           setTimeout(function(){
	                        	   $("#security_blank_tb_id").append(dataStr);
	                               // 每次数据加载完，必须重置
	                               me.resetload();
	                           },1000);
