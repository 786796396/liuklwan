    ...
    function initSecurityResponseData(){
        //清空列表数据
        $("#security_response_id").html("");
        // dropload
        $("#security_response_list_id").dropload({
            scrollArea : window,
            loadDownFn : function(me){
					$.ajax({
				   	 	type:"post",
				     	async:false,
				        url: webPath+"/advanceToken_getsecurityResponseOrgData.action?siteCode="+siteCode
				           ...
				           if(resultMap.errormsg){
				        	   ...
				        	   if(returnList && returnList.length>0){
				        		   for ( var i = 0; i < returnList.length; i++) {
				        			   dataStr+="<li onclick='goToSecurityResponseOrg(\""+returnList[i]["siteCode"]+"\","+returnList[i]["problemNum"]+",\""+returnList[i]["siteName"]+"\")'>"
		                           ...
		                           // 为了测试，延迟1秒加载
		                           setTimeout(function(){
		                        	   $("#security_response_id").append(dataStr);
		                               // 每次数据加载完，必须重置
		                               me.resetload();
		                           },1000);
    ...
    //跳转到填报单位
    function goToSecurityResponseOrg(code,problemNum,name){
		window.location.href=webPath+"/advanceToken_securityResponseTbIndex.action?siteCode="+code
		+"&securityResponse="+problemNum+"&siteName="+name
