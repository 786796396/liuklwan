    ...
    function initSecurityServiceTbData(){
        //清空列表数据
        $("#security_service_tb_id").html("");
        // dropload
        $("#security_service_tb_list_id").dropload({
            scrollArea : window,
            loadDownFn : function(me){
				$.ajax({
			   	 	type:"post",
			     	async:false,
			        url: webPath+"/advanceToken_getSecurityServiceTbData.action?siteCode="+siteCode
			        	   ...
			        	   if(returnList && returnList.length>0){
			        		   for ( var i = 0; i < returnList.length; i++) {
			        			   dataStr+="<li class='clearfix'><i class='fl'></i><div class='fl right-part'><div class='clearfix'>"
			        				   +"<h4 class='fl'>"+returnList[i]["serviceItem"]+"</h4>"
			        				   ...
			        				   +"<p class='clearfix'><span class='title-txt-f fl'>类型\：</span><span class='fl except-tit'>"+returnList[i]["problemTypeName"]+"</span></p>"
	                           ...
	                           // 为了测试，延迟1秒加载
	                           setTimeout(function(){
	                        	   $("#security_service_tb_id").append(dataStr);
	                               // 每次数据加载完，必须重置
	                               me.resetload();
	                           },1000);
			        	   }else{
