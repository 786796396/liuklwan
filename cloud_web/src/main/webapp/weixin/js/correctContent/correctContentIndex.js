    ...
    $("#correct_content_id").dropload({
        ...
        loadDownFn : function(me){
        	...
        	$.ajax({
           	 	type:"post",
             	async:false,
                url: webPath+"/token_getCorrectContent.action",
                dataType:"json",
    	        data: JSON.stringify(jsonObj),
    	        contentType: "application/json",
                success : function(data){
                	if(data.errorMsg){
                        $("#correct_content_id").find(".dropload-load").addClass("hide");
                        $("#conn_table_hide").attr("style","display:block");
                        $("#conn_table_hide").html(data.errorMsg);
                        
                	}else{
                		$("#correct_content_id").find(".dropload-load").removeClass("hide");
                        ...
                        if(resultList){
    						...
    						$("#conn_table_hide").attr("style","display:none");
    						//显示列表
    						$("#correct_content_table_id").attr("style","display:block");
                    		for(var i =0; i <resultList.length; i++){
                    			...
                    			if(url.substring(0,4)!="http"){
            				 	...
            				 	//头标题
            				 	listStr+="<h3 class='tit-h3-2'><i class='i-point'></i>错别字<span class='pull-right'>"+resultList[i].scanDate+"</span></h3>";
            				 	//封装里面div信息
            				 	listStr+="<div class='table-box1'>"+
            	            			...
            	            			"<div class='row'><label>错误类型：</label><span class='ellipsis col-xs-8 col-sm-9 col-md-9'>&nbsp;"+resultList[i].correctType+"</span></div>"+
            	            			"<div class='row'><label>疑似错误：</label><span class='ellipsis col-xs-8 col-sm-9 col-md-9'>&nbsp;"+resultList[i].wrongTerm+"</span></div>"+
            	            			"<div class='row'><label>推荐修改：</label><span class='ellipsis col-xs-8 col-sm-9 col-md-9'>&nbsp;"+resultList[i].expectTerms+"</span></div>"+
            	            			"<div class='row'><label>页面URL：</label><a class='ellipsis col-xs-8 col-sm-9 col-md-9' target='_blank' href='"+url+"'>&nbsp;"+url+"</a></div>"+
    									"</div>";
                            ...
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
                                $("#correct_content_table_id").append(listStr);
                                // 每次数据加载完，必须重置
                                me.resetload();
                            },1000);
                        }
