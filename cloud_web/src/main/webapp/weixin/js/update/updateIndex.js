    ...
    $("#update_id").dropload({
        ...
        loadDownFn : function(me){
        	...
        	$.ajax({
           	 	type:"post",
             	async:false,
                url: webPath+"/token_getUpdate.action",
                dataType:"json",
    	        data: JSON.stringify(jsonObj),
    	        contentType: "application/json",
                success : function(data){
                	if(data.errorMsg){
                        $("#update_id").find(".dropload-load").addClass("hide");
                        $("#conn_table_hide").attr("style","display:block");
                        $("#conn_table_hide").html(data.errorMsg);
                        
                	}else{
                		$("#update_id").find(".dropload-load").removeClass("hide");
                        ...
                        if(resultList){
    						...
    						$("#conn_table_hide").attr("style","display:none");
    						//显示列表
    						$("#update_table_id").attr("style","display:block");
                    		for(var i =0; i <resultList.length; i++){
                    			...
                    			if(url.substring(0,4)!="http"){
            				 	...
            				 	//头标题
            				 	listStr+="<h3 class='tit-h3-2'><i class='i-point'></i>内容更新<span class='pull-right'>"+resultList[i].scanDate+"</span></h3>";
            				 	//封装里面div信息
            				 	listStr+="<div class='table-box1'>"+
            	            			...
            	            			"<div class='row'><label>标题：</label><span class='ellipsis col-xs-8 col-sm-9 col-md-9'>&nbsp;"+resultList[i].title+"</span></div>"+
            	            			"<div class='row'><label>url：</label><a class='ellipsis col-xs-8 col-sm-9 col-md-9' target='_blank' href='"+url+"'>"+url+"</a></div>"+
            	            			"<div class='row'><label>更新时间：</label><span class='ellipsis col-xs-8 col-sm-9 col-md-9'>&nbsp;"+resultList[i].updateTime+"</span></div>"+
            	            			"</div>";
                            ...
                            // 为了测试，延迟1秒加载
                            setTimeout(function(){
                                $("#update_table_id").append(listStr);
                                // 每次数据加载完，必须重置
                                me.resetload();
                            },1000);
                        }
